...
export const SettingsView: React.FC<Props> = ({
  ...
  return (
    <div id="settings-container" className="w-full max-w-5xl flex flex-col p-2 sm:p-4 mx-auto space-y-6">
      ...
      {/* 5. Android APK Build & Export Hub */}
      <div className="bg-gradient-to-br from-emerald-950/40 via-slate-900 to-slate-900 rounded-3xl p-6 border border-emerald-500/30 shadow-xl space-y-4">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-lg font-bold text-white flex items-center gap-2">
                <span>{isRtl ? 'ساخت و خروجی اپلیکیشن اندروید (APK / AAB)' : 'Android APK Build & Export Hub'}</span>
                <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-900 text-emerald-300 border border-emerald-500/50">
                  Capacitor Ready
                </span>
              </h2>
              <p className="text-xs text-slate-400">
                {isRtl ? 'پروژه به صورت کامل برای ساخت فایل نصبی APK برای گوشی‌های هوشمند و مانیتورهای خودرو (Android Head Unit) پیکربندی شده است.' : 'Fully configured with native Android project, Bluetooth BLE permissions, and automated build workflows.'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
          {/* Method 1: GitHub Actions CI (Automatic) */}
          <div className="p-4 bg-slate-950/90 rounded-2xl border border-emerald-500/30 flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-2 text-emerald-400 font-bold text-xs mb-2">
                <Download className="w-4 h-4" />
                <span>{isRtl ? 'روش اول: ساخت خودکار APK در گیت‌هاب' : 'Method 1: GitHub Actions (1-Click)'}</span>
              </div>
              <p className="text-xs text-slate-300 leading-relaxed mb-3">
                {isRtl 
                  ? 'فایل ورک‌فلو `.github/workflows/build-apk.yml` آماده است. با پوش به مخزن یا زدن Run workflow در بخش Actions، فایل APK به صورت خودکار کامپایل شده و در بخش Artifacts قابل دانلود است.'
                  : 'Workflow `.github/workflows/build-apk.yml` builds debug and release APKs automatically upon push or manual dispatch.'}
              </p>
            </div>
            <div className="p-2.5 bg-slate-900 rounded-xl font-mono text-[11px] text-emerald-300 border border-emerald-900/50">
              Artifacts: MultiGauge-OBD-Debug-APK
            </div>
          </div>
          ...
          <div className="p-4 bg-slate-950/90 rounded-2xl border border-slate-800 flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-2 text-cyan-400 font-bold text-xs mb-2">
                ...
                <span>{isRtl ? 'روش دوم: دستورات ساخت محلی و خروجی' : 'Method 2: Local CLI & Android Studio'}</span>
              </div>
              <p className="text-xs text-slate-300 leading-relaxed mb-3">
                {isRtl 
                  ? 'برای باز کردن پروژه در Android Studio یا خروجی با خط فرمان در سیستم خود:'
                  : 'Run local build commands to open in Android Studio or compile with Gradle:'}
              </p>
            </div>
            <div className="space-y-1.5 p-2.5 bg-slate-900 rounded-xl font-mono text-[10px] text-slate-300 border border-slate-800">
              <div className="text-cyan-400 font-bold">npm run build:android</div>
              <div className="text-slate-400">npx cap open android</div>
              <div className="text-amber-400">cd android && ./gradlew assembleDebug</div>
            </div>
          </div>

          {/* Method 3: Instant PWA Install on Car Unit */}
          <div className="p-4 bg-slate-950/90 rounded-2xl border border-slate-800 flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-2 text-sky-400 font-bold text-xs mb-2">
                <Layers className="w-4 h-4" />
                <span>{isRtl ? 'روش سوم: نصب مستقیم وب‌اپلیکیشن (PWA)' : 'Method 3: Direct PWA Install'}</span>
              </div>
              <p className="text-xs text-slate-300 leading-relaxed mb-3">
                {isRtl 
                  ? 'در مرورگر کروم گوشی یا مانیتور اندرویدی خودرو، دکمه ۳ نقطه بالا را بزنید و «Install App» یا «Add to Home Screen» را لمس کنید تا مانند یک اپلیکیشن محلی تمام‌صفحه و آفلاین نصب شود.'
                  : 'Tap Chrome menu > "Install App" or "Add to Home screen" on Android phone or head unit for standalone full-screen operation.'}
              </p>
            </div>
            <div className="p-2.5 bg-slate-900 rounded-xl font-mono text-[11px] text-sky-300 border border-slate-800 flex items-center justify-between">
              <span>Full Screen & Offline Cache</span>
              <Check className="w-4 h-4 text-emerald-400" />
            </div>
          </div>
        </div>

        <div className="p-3.5 bg-slate-950/60 rounded-2xl border border-emerald-900/30 flex items-center justify-between flex-wrap gap-2 text-xs">
          <div className="flex items-center gap-2 text-slate-300">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            <span>
              {isRtl 
                ? 'پکیج نام اندروید: com.multigauge.obd | کلیه دسترسی‌های بلوتوث کم‌مصرف (BLE Scan & Connect) و وای‌فای در AndroidManifest تنظیم شده‌اند.'
                : 'Package: com.multigauge.obd | BLE Scan, Connect & Location permissions included in AndroidManifest.'}
            </span>
          </div>
        </div>
      </div>

      {/* 6. iOS & Offline Execution Guide */}
