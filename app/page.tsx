...
export default function Home() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${base}database/standard_pid.json`).then((r)=>r.json() as Promise<PIDDefinition[]>),
      fetch(`${base}database/dtc_codes.json`).then((r)=>r.json() as Promise<DTCRecord[]>),
      loadPreference<Language>('language','en'),
      loadPreference<Theme>('theme','professional'),
      loadPreference<string[]>('visible',['rpm','speed','coolant','engineLoad'])
    ]).then(([pidData,dtcData,savedLanguage,savedTheme,savedVisible])=>{
      setPids(pidData);
      setCatalog(dtcData);
      setLanguage(savedLanguage);
      setTheme(savedTheme);
      setVisible(savedVisible);
      void connect('simulator');
    });
